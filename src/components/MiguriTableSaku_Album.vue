...
let center = []
let w_center = ['藤吉　夏鈴', '山﨑　天']
let row_1 = ['山下　瞳月', "田村　保乃", "守屋　麗奈", "村井　優",]
let row_2 = ['大園　玲', '松田　里奈', '石森　璃花', '谷口　愛季', '中嶋　優月', '的野　美青', '向井　純葉', '村山　美羽', '森田　ひかる']
let row_3 = ['井上　梨名', '遠藤　光莉', '大沼　晶保', '幸阪　茉里乃', '武元　唯衣', '増本　綺良', '遠藤　理子', "小田倉　麗奈", '小島　凪紗']
...
<template>
  ...
  <a :href="`https://fortunemusic.jp/sakurazaka_202504/${entry + 1}/goods_list/`">https://fortunemusic.jp/sakurazaka_202504/{{
    entry + 1 }}/goods_list/</a><br><br>
  <!-- #姫奈おめでとう<br>
  #やっぱり5期生が大好きなんですよ<br> -->
  櫻坂46 {{ single }}{{ single_end }}アルバム『{{ single_title }}』ミーグリ {{ entry }}次抽選結果<br>
          ...
          <template v-for="ki in [2, 3]">
            ...
            <tr v-for="(member, index) in parsed.filter(member => getKi[member.name] == ki)" :key="member" class="">
              ...
              <td class="left-align left-border" :class="{ 'lighter-pink-bg': member.sold === member.available }">
                ...
                <span>{{ w_center.includes(member.name) ? "/ Wセンター" : "" }}</span>
